...
from hellspam import SpamBot1
from pathlib import Path
from telethon import events
from hellspam.__main__ import load_plugs
...
async def install(e):
    if e.sender_id in LIMIT:
        ...
        if e.reply_to_msg_id:
            ...
            try:
                message = await e.client.download_media(module, "./hellspam/plugins/")
                if "(" not in message:
                    mypath = Path(message)
                    name = mypath.stem
...
async def uninstall(e):
    if e.sender_id in LIMIT:
        text = e.raw_text[11:]
        mypath = f"./hellspam/plugins/{text}.py"
        try:
            os.remove(mypath)
